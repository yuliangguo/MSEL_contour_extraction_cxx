...
//        edge_map->insert(new dbdet_edgel(vgl_point_2d<double>(x, y), dir, conf, 0.0, uncer), ix, iy);
//      else
        edge_map->insert(new dbdet_edgel(vgl_point_2d<double>(x, y), dir, conf, 0.0, uncer));
    }
    else { //pixel edges  
      //create an edgel token
      edge_map->insert(new dbdet_edgel(vgl_point_2d<double>(ix, iy), idir, iconf, 0.0, uncer));
    }
  }
  infp.close();
