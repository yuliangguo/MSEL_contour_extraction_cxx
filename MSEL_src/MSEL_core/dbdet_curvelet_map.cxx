#include "dbdet_curvelet_map.h"

#include "dbdet_curvelet.h"
#include "dbdet_edgemap.h"

//: constructor
dbdet_curvelet_map::dbdet_curvelet_map(dbdet_edgemap_sptr EM,
                                       dbdet_curvelet_params params): 
  EM_(EM), params_(params), map_(0)
{}

//: destructor
dbdet_curvelet_map::~dbdet_curvelet_map()
{
  clear(); //delete everything upon exit
  EM_=0; //delete the reference to the edgemap
}

//: resize the graph
void dbdet_curvelet_map::resize(unsigned size)
{ 
  if (size!=map_.size())
    clear();

  map_.resize(size);
  map2_.resize(size);
}

//: clear the graph
void dbdet_curvelet_map::clear()
{
  clear_all_curvelets();
  map_.clear();
  map2_.clear();
}

//: clear all the curvelets in the graph
void dbdet_curvelet_map::clear_all_curvelets()
{
  vcl_set<dbdet_curvelet*> deleted;
  //delete all the curvelets in the map
  for (unsigned i=0; i<map_.size(); i++)
  {
    //delete all the curvelets formed by this edgel
    curvelet_list_iter p_it = map_[i].begin();
    for (; p_it != map_[i].end(); p_it++) {
      if (deleted.find(*p_it) == deleted.end()) {
        delete (*p_it);
        deleted.insert(*p_it);
      }
    }

    //delete all the curvelets formed by this edgel
    curvelet_list_iter p_it2 = map2_[i].begin();
    for (; p_it2 != map2_[i].end(); p_it2++) {
      if (deleted.find(*p_it2) == deleted.end()) {
        delete (*p_it2);
        deleted.insert(*p_it2);
      }
    }

    map_[i].clear();
    map2_[i].clear();
  }
}

//: add a curvelet to this edgel
void dbdet_curvelet_map::add_curvelet(dbdet_curvelet* curvelet, bool dir)
{
  if (dir)
    map_[curvelet->ref_edgel->id].push_back(curvelet);
  else
    map2_[curvelet->ref_edgel->id].push_back(curvelet);
}

//: remove a curvelet from this edgel
void dbdet_curvelet_map::remove_curvelet(dbdet_curvelet* curvelet)
{
  //go over the list of curvelets of the ref edgel
  curvelet_list_iter c_it = map_[curvelet->ref_edgel->id].begin();
  for (; c_it != map_[curvelet->ref_edgel->id].end(); c_it++){
    if (curvelet == (*c_it)){
      map_[curvelet->ref_edgel->id].erase(c_it);
      return;
    }
  }

  //also search the other map if the curvelet has not been found
  curvelet_list_iter c_it2 = map2_[curvelet->ref_edgel->id].begin();
  for (; c_it2 != map2_[curvelet->ref_edgel->id].end(); c_it2++){
    if (curvelet == (*c_it2)){
      map2_[curvelet->ref_edgel->id].erase(c_it2);
      return;
    }
  }
}

//: delete all the curvelets formed by this edgel
void dbdet_curvelet_map::delete_all_curvelets(dbdet_edgel* e)
{
  curvelet_list_iter p_it = map_[e->id].begin();
  for (; p_it != map_[e->id].end(); p_it++)
    delete (*p_it);

  //delete all the curvelets formed by this edgel
  curvelet_list_iter p_it2 = map2_[e->id].begin();
  for (; p_it2 != map2_[e->id].end(); p_it2++)
    delete (*p_it2);

  map_[e->id].clear();
  map2_[e->id].clear();
}

//: does the current curvelet exist?
dbdet_curvelet* dbdet_curvelet_map::does_curvelet_exist(dbdet_edgel* e, vcl_deque<dbdet_edgel*> & chain)
{
  //go over all the curvelets of the current size formed by the current edgel
  curvelet_list_iter cv_it = map_[e->id].begin();
  for (; cv_it != map_[e->id].end(); cv_it++){
    dbdet_curvelet* cvlet = (*cv_it);

    if (cvlet->edgel_chain.size() != chain.size())
      continue;

    bool cvlet_exists = true; //reset flag
    for (unsigned k=0; k<chain.size(); k++)
      cvlet_exists = cvlet_exists && (cvlet->edgel_chain[k]==chain[k]);

    //the flag will remain true only if all the edgels match
    if (cvlet_exists)
      return cvlet; //return matching curvelet
  }

  return 0; //curvelet does not exist
}

//: does the given edgel pair exist on the ref edgel?
dbdet_curvelet* dbdet_curvelet_map::find_pair(dbdet_edgel* ref, dbdet_edgel* eA, dbdet_edgel* eB)
{
  //go over all the pairs formed by this edgel
  curvelet_list_iter p_it = map_[ref->id].begin();
  for (; p_it != map_[ref->id].end(); p_it++){
    dbdet_curvelet* p1 = (*p_it);
    if (p1->edgel_chain.size()==2 && 
        p1->edgel_chain[0]==eA && p1->edgel_chain[1]==eB)
      return p1; //pair found
  }
  return 0; //not found
}

//: does the given triplet exist on the ref edgel?
dbdet_curvelet* dbdet_curvelet_map::find_triplet(dbdet_edgel* ref, dbdet_edgel* eA, dbdet_edgel* eB, dbdet_edgel* eC)
{
  //go over all the triplets formed by this edgel
  curvelet_list_iter t_it = map_[ref->id].begin();
  for (; t_it != map_[ref->id].end(); t_it++){
    dbdet_curvelet* t1 = (*t_it);
    if (t1->edgel_chain.size()==3 &&
        t1->edgel_chain[0]==eA && t1->edgel_chain[1]==eB && t1->edgel_chain[2]==eC)
      return t1; //triplet found
  }
  return 0; //not found
}

//: return largest curvelet formed at the given edgel
dbdet_curvelet* dbdet_curvelet_map::largest_curvelet(dbdet_edgel* e)
{
  unsigned largest_size = 0;
  dbdet_curvelet* largest_cvlet = 0;

  curvelet_list_iter cv_it = map_[e->id].begin();
  for ( ; cv_it!=map_[e->id].end(); cv_it++)
  {
    dbdet_curvelet* cvlet = (*cv_it);
    if (cvlet->edgel_chain.size()>largest_size){
      largest_size = cvlet->edgel_chain.size();
      largest_cvlet = cvlet;
    }
  }
  return largest_cvlet;
}


