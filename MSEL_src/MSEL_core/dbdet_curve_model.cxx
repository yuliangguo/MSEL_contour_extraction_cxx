...
dbdet_simple_linear_curve_model::dbdet_simple_linear_curve_model(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                                                                 double dpos, double dtheta, 
                                                                 double token_len, double /*max_k*/, double /*max_gamma*/,
                                                                 bool /*adaptive*/):
  dbdet_linear_curve_model_base(ref_e->pt, ref_e->tangent), min_theta(0.0), max_theta(0.0)
{
  type = LINEAR;
...
dbdet_simple_linear_curve_model::dbdet_simple_linear_curve_model(dbdet_curve_model* cm1, dbdet_curve_model* cm2):
  dbdet_linear_curve_model_base(), min_theta(0.0), max_theta(0.0)
{
  type = LINEAR;
  pt = ((dbdet_linear_curve_model_base*)cm1)->pt;
  theta = ((dbdet_linear_curve_model_base*)cm1)->theta; //default

  //compute new curve bundle
  dbdet_intersect_angle_range(((dbdet_simple_linear_curve_model*)cm1)->min_theta, ((dbdet_simple_linear_curve_model*)cm1)->max_theta,
                              ((dbdet_simple_linear_curve_model*)cm2)->min_theta, ((dbdet_simple_linear_curve_model*)cm2)->max_theta, 
                              min_theta, max_theta);
...
dbdet_simple_linear_curve_model* dbdet_simple_linear_curve_model::intersect(dbdet_simple_linear_curve_model* cm)
{
  return new dbdet_simple_linear_curve_model(this, cm);
}

//: determine if edgel pair is legal
bool dbdet_simple_linear_curve_model::edgel_pair_legal(dbdet_int_params &params, double tan1, double tan2)
{
...
void dbdet_simple_linear_curve_model::
compute_curve_bundle(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                     double dpos, double dtheta, double /*token_len*/)
{
  ...
  if (edgel_pair_legal(params, e1->tangent, e2->tangent))
  {
    ...
    if (ref_e==e1)
    {
      ...
      // FIX ME!
      if (params.d < dpos){
        min_theta = dbdet_angle0To2Pi(e1->tangent - dtheta);
        max_theta = dbdet_angle0To2Pi(e1->tangent + dtheta);
        return;
      }
      ...
      if (dbdet_intersect_angle_range(params.t2-dtheta, params.t2+dtheta, -alpha, alpha, a1, b1)){
        //intersected with domain 3
        if (dbdet_intersect_angle_range(a1, b1, params.t1-dtheta, params.t1+dtheta, a2, b2))
        {
          min_theta = dbdet_angle0To2Pi(e1->tangent + a2 - params.t1);
          max_theta = dbdet_angle0To2Pi(e1->tangent + b2 - params.t1);
        }
      }
    }
    else {
      ...
      // FIX ME!
      if (params.d < dpos){
        min_theta = dbdet_angle0To2Pi(e2->tangent - dtheta);
        max_theta = dbdet_angle0To2Pi(e2->tangent + dtheta);
        return;
      }
      ...
      double a1, b1, a2, b2;
      //domain 1 intersected with domain 2
      if (dbdet_intersect_angle_range(params.t1-dtheta, params.t1+dtheta, -alpha, alpha, a1, b1)){
        //intersected with domain 3
        if (dbdet_intersect_angle_range(a1, b1, params.t2-dtheta, params.t2+dtheta, a2, b2))
        {
          min_theta = dbdet_angle0To2Pi(e2->tangent + a2 - params.t2);
          max_theta = dbdet_angle0To2Pi(e2->tangent + b2 - params.t2);
        }
      }
    }
  }
...
vgl_point_2d<double>  dbdet_simple_linear_curve_model::compute_best_fit(vcl_deque<dbdet_edgel*> &/*edgel_chain*/)
{
  ...
  if (max_theta<min_theta)
    theta = dbdet_angle0To2Pi((max_theta+2*vnl_math::pi+min_theta)/2); //the range straddles the 0-crossing
  else
...
void dbdet_simple_linear_curve_model::report_accuracy(double *estimates, double *min_estimates, double *max_estimates)
{
...
//: Constructor 1: From a pair of edgels
dbdet_linear_curve_model::dbdet_linear_curve_model(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                                                   double dpos, double dtheta, 
                                                   double token_len, double /*max_k*/, double /*max_gamma*/,
                                                   bool /*adaptive*/):
dbdet_linear_curve_model_base(ref_e->pt, ref_e->tangent), ref_pt(ref_e->pt), ref_theta(ref_e->tangent)
{
  type = LINEAR;
...
//: constructor 3: From the intersection of two curve bundles
dbdet_linear_curve_model::dbdet_linear_curve_model(dbdet_curve_model* cm1, dbdet_curve_model* cm2):
dbdet_linear_curve_model_base()
{
  type = LINEAR;
  
  ref_pt = ((dbdet_linear_curve_model*)cm1)->ref_pt;
  ref_theta = ((dbdet_linear_curve_model*)cm1)->ref_theta;

  pt = ((dbdet_linear_curve_model_base*)cm1)->pt;
  theta = ((dbdet_linear_curve_model_base*)cm1)->theta; //default

  //compute new curve bundle
  cv_bundle = vgl_clip(((dbdet_linear_curve_model*)cm1)->cv_bundle, ((dbdet_linear_curve_model*)cm2)->cv_bundle, vgl_clip_type_intersect);
}

//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_linear_curve_model* dbdet_linear_curve_model::intersect(dbdet_linear_curve_model * cm)
{
  return new dbdet_linear_curve_model(this, cm);
}

//: determine if edgel pair is legal
bool dbdet_linear_curve_model::edgel_pair_legal(dbdet_int_params &params, double tan1, double tan2)
{
  //simple orientation based pruning
  bool ret = (dbdet_dot(params.ref_dir, tan1)>0 && dbdet_dot(params.ref_dir, tan2)>0);
  return ret;
                     ...
                     double dpos, double dtheta, double /*token_len*/)
{
  ...
  if (ref_e==e1)
  {
    ...
    double a1, b1, a2, b2;
    //domain 1 intersected with domain 2
    if (dbdet_intersect_angle_range_mpi_pi(-dtheta, dtheta, -params.t1-alpha, -params.t1+alpha, a1, b1)){
      //intersected with domain 3
      if (dbdet_intersect_angle_range_mpi_pi(a1, b1, params.t2-params.t1-dtheta, params.t2-params.t1+dtheta, a2, b2))
      {
        cv_bundle.new_sheet();
    ...
    double a1, b1, a2, b2;
    //domain 1 intersected with domain 2
    if (dbdet_intersect_angle_range_mpi_pi(-dtheta, dtheta, -params.t2-alpha, -params.t2+alpha, a1, b1)){
      //intersected with domain 3
      if (dbdet_intersect_angle_range_mpi_pi(a1, b1, params.t1-params.t2-dtheta, params.t1-params.t2+dtheta, a2, b2))
      {
        cv_bundle.new_sheet();
        cv_bundle.push_back( dpos, a2+alpha);
        cv_bundle.push_back(-dpos, a2-alpha);
...
//: report accuracy of measurement
void dbdet_linear_curve_model::report_accuracy(double *estimates, double *min_estimates, double *max_estimates)
{
...
//: Constructor 1: From a pair of edgels
dbdet_CC_curve_model::dbdet_CC_curve_model(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                                           double dpos, double dtheta, 
                                           double token_len, double max_k, double /*max_gamma*/,
                                           bool /*adaptive*/):
...
//: constructor 3: From the intersection of two curve bundles
dbdet_CC_curve_model::dbdet_CC_curve_model(dbdet_curve_model* cm1, dbdet_curve_model* cm2)
{
  type = CC;
  pt = ((dbdet_CC_curve_model*)cm1)->pt;
  tangent = ((dbdet_CC_curve_model*)cm1)->tangent;

  cv_bundle = vgl_clip(((dbdet_CC_curve_model*)cm1)->cv_bundle, ((dbdet_CC_curve_model*)cm2)->cv_bundle, vgl_clip_type_intersect);
}

//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_CC_curve_model* dbdet_CC_curve_model::intersect(dbdet_CC_curve_model* cm)
{
  return new dbdet_CC_curve_model(this, cm);
}

//: determine if edgel pair is legal
bool dbdet_CC_curve_model::edgel_pair_legal(dbdet_int_params &params, double tan1, double tan2)
{
  //simple orientation based pruning
  bool ret = (dbdet_dot(params.ref_dir, tan1)>0 && dbdet_dot(params.ref_dir, tan2)>0);
  return ret;
...
void dbdet_CC_curve_model::
compute_curve_bundle(vgl_polygon<double>& bundle, dbdet_int_params &params, 
                     ...
                     double token_len, double max_k)
{
  ...
  if (first_is_ref)
  {
    ...
    for (int i=0; i<5; i++)
    {
      ...
      dbdet_intersect_angle_range_mpi_pi(t1_min, t1_max, t1-alpha, t1+alpha, t1_sol_min , t1_sol_max);
  ...
  else //e2 is the ref
  {
    ...
    for (int i=0; i<5; i++)
    {
      ...
      dbdet_intersect_angle_range_mpi_pi(t2_min, t2_max, t2-alpha, t2+alpha, t2_sol_min , t2_sol_max);
...
//: report accuracy of measurement
void dbdet_CC_curve_model::report_accuracy(double *estimates, double *min_estimates, double *max_estimates)
{
...
//: copy constructor
dbdet_CC_curve_model_new::dbdet_CC_curve_model_new(const dbdet_CC_curve_model_new& other) : 
  dbdet_curve_model(other) 
{ 
  ...
  //: Constructor 1: From a pair of edgels
dbdet_CC_curve_model_new::
dbdet_CC_curve_model_new(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                         double dpos, double dtheta, 
                         double token_len, double max_k, double /*max_gamma*/,
                         bool /*adaptive*/):
...
dbdet_CC_curve_model_new::
dbdet_CC_curve_model_new(dbdet_CC_curve_model_new* cm, vgl_point_2d<double> pt, double theta):
  pt(0.0,0.0), theta(0), k(0.0)
{
  type = CC2;
  ...
  for (int i=0; i<NkClasses; i++){
    bool sgn_change;
    vgl_polygon<double> poly =  transport_bundle_to(cm->cv_bundles[i], dbdet_k_classes[i], pt, theta, sgn_change);
    if (sgn_change)
...
//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_CC_curve_model_new* dbdet_CC_curve_model_new::intersect(dbdet_CC_curve_model_new* cm)
{
  assert(cm->type == CC2);
  return new dbdet_CC_curve_model_new(this, cm);
}

//: construct and return a curve model of the same type by transporting it to a new extrinsic anchor
dbdet_curve_model* dbdet_CC_curve_model_new::transport(vgl_point_2d<double> pt, double theta)
{
  return new dbdet_CC_curve_model_new(this, pt, theta);
}

//: construct and return a curve model of the same type by looking for legal transitions with another curve bundle
dbdet_curve_model* dbdet_CC_curve_model_new::transition(dbdet_curve_model* cm, int& trans_k, int& trans_type, double & bundle_size)
{
  if (cm->type!=CC2)
    return 0; //type mismatch

  dbdet_CC_curve_model_new* next_cm = (dbdet_CC_curve_model_new*)cm; //typecast for easy handling
  ...
  if (trans_k>=0){ //if any legal ones
    ...
    dbdet_CC_curve_model_new* new_cm = new dbdet_CC_curve_model_new(ref_pt, ref_theta);
...
dbdet_curve_model* dbdet_CC_curve_model_new::consistent_transition(dbdet_curve_model* cm, int& trans_k, int& trans_type, double &gc_cost)
{
  if (cm->type!=CC2)
    return 0; //type mismatch
...
dbdet_curve_model* dbdet_CC_curve_model_new::C1_transition(dbdet_curve_model* cm)
{
  ...
  dbdet_CC_curve_model_new* new_cm = new dbdet_CC_curve_model_new(next_cm->ref_pt, next_cm->ref_theta);
...
bool dbdet_CC_curve_model_new::bundle_is_valid() 
{ 
  bool valid = false;
...
//: determine if edgel pair is legal
bool dbdet_CC_curve_model_new::edgel_pair_legal(dbdet_int_params &params, double tan1, double tan2)
{
  //simple orientation based pruning
  bool ret = (dbdet_dot(params.ref_dir, tan1)>0 && dbdet_dot(params.ref_dir, tan2)>0);
  return ret;
                     ...
                     double dpos, double dtheta, double token_len, double /*max_k*/)
{
  ...
  dbdet_edgel* neigh_e=0; //relic of older design req. 
  if (ref_e == e1)
    neigh_e = e2;
  else
    neigh_e = e1;
  ...
  for (int i=0; i<NkClasses; i++){
    if (vcl_fabs(dbdet_k_classes[i])<dk){ //only the legal curve classes
      ...
      bool sgn_change;
      vgl_polygon<double> trans_CB = transport_bundle_from(omega, dbdet_k_classes[i], neigh_e->pt, neigh_e->tangent, sgn_change);
...
vgl_polygon<double> dbdet_CC_curve_model_new::
...
vgl_point_2d<double> dbdet_CC_curve_model_new::transport_CC(vgl_point_2d<double>dx_dt, double k,
...
//: Compute the best fit curve from the curve bundle  
vgl_point_2d<double> dbdet_CC_curve_model_new::compute_best_fit()
{
  ...
  ////choose the curve class with the most flexibility
  //// assign the centroid of its bundle as the best fit curve
  //int best_k_ind = -1;
  //double max_area=0;
  //for (int i=0; i<NkClasses; i++){
  //  if (cv_bundles[i].num_sheets()==1){
  //    double a = vgl_area(cv_bundles[i]);
  //    if (a>max_area){
  //      best_k_ind=i;
  //      max_area = a;
  //    }
...
void dbdet_CC_curve_model_new::set_best_fit(vgl_point_2d<double> dx_dt, double kk)
{
  double dx = dx_dt.x();
  double dt = dx_dt.y();
...
//----------------------------------------------------------------------------
// Old version of dbdet_CC_curve_model_new
//----------------------------------------------------------------------------
...
//  //transport each bundle
//  for (int i=0; i<NkClasses; i++)
//    cv_bundles[i] = transport_bundle_to(cm->cv_bundles[i], dbdet_k_classes[i], pt, theta);
//
//  //Now update the extrinsic refs
//  ref_pt = pt;
//  ref_theta = theta;
...
//
////: construct and return a curve model of the same type by intersecting with another curve bundle
//dbdet_curve_model* dbdet_CC_curve_model_new::intersect(dbdet_curve_model* cm)
//{
//  if (cm->type!=CC2)
//    return 0; //type mismatch
//
//  return new dbdet_CC_curve_model_new(this, (dbdet_CC_curve_model_new*)cm);
//}
//
////: construct and return a curve model of the same type by transporting it to a new extrinsic anchor
//dbdet_curve_model* dbdet_CC_curve_model_new::transport(vgl_point_2d<double> pt, double theta)
...
////: Transport curve bundle from the given extrinsic point-tangent to the current point-tanget for the given curve model
//vgl_polygon<double> dbdet_CC_curve_model_new::
//transport_bundle_from(const vgl_polygon<double> & cv_bundle, double k, vgl_point_2d<double> pt, double theta)
//{
//  return transport_bundle(cv_bundle, k, pt, theta, ref_pt, ref_theta);
//}
//
////: Transport curve bundle from the given extrinsic point-tangent to the current point-tanget for the given curve model
//vgl_polygon<double> dbdet_CC_curve_model_new::
//transport_bundle_to(const vgl_polygon<double> & cv_bundle, double k, vgl_point_2d<double> pt, double theta)
//{
//  return transport_bundle(cv_bundle, k, ref_pt, ref_theta, pt, theta);
//}
//
////: Transport a curve bundle from one anchor to the current anchor
//vgl_polygon<double> dbdet_CC_curve_model_new::
...
//  if (cv_bundle.num_sheets()!=1) return trans_CB;
//
//  // Procedure to transport a bundle
//  // (a) First find the sub-bundle that is transportable
//  // (b) intersect the given bundle with the legal bundle
//  // (c) transport the remaining to the new extrinsic point-tangent
//
//  // (a)
//  double DX = dpt.x()-spt.x();
//  double DY = dpt.y()-spt.y();
...
//  }
//
//  vgl_polygon<double> DX_DT; //CCW
//  DX_DT.new_sheet();
//  DX_DT.push_back(dx1_min, dt1_min_b);
//  DX_DT.push_back(dx1_max, dt1_max_b);
//  DX_DT.push_back(dx1_max, dt1_max);
//  DX_DT.push_back(dx1_min, dt1_min);
//
//  // (b)
//  vgl_polygon<double> valid_CB = vgl_clip(cv_bundle, DX_DT);
//
//  if (valid_CB.num_sheets()!=1) //no part of the CB can be transported
//    return trans_CB;
//
//  // (c) transport the convex hull of the remaining bundle (valid_CB)
//  trans_CB.new_sheet();
//  bool valid = true;
//  for (unsigned i=0; i<valid_CB[0].size(); i++){
//    trans_CB.push_back(transport_CC(valid_CB[0][i], k, spt, stheta, dpt, dtheta, valid));
//
//    if (!valid){ //invalid point on the bundle CHull. Declare this as an invalid bundle
//      trans_CB.clear();
//      break;
//    }
//  }
...
//: Constructor 1: From a pair of edgels
dbdet_CC_curve_model_perturbed::dbdet_CC_curve_model_perturbed(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                                                               double dpos, double dtheta, 
                                                               double token_len, double max_k, double /*max_gamma*/,
                                                               bool /*adaptive*/):
  dbdet_CC_curve_model(ref_e->pt, ref_e->tangent)
{
  type = CC;
...
//: constructor 3: From the intersection of two curve bundles
dbdet_CC_curve_model_perturbed::dbdet_CC_curve_model_perturbed(dbdet_curve_model* cm1, dbdet_curve_model* cm2)
{
  type = CC;

  pt = ((dbdet_CC_curve_model_perturbed*)cm1)->pt;
  tangent = ((dbdet_CC_curve_model_perturbed*)cm1)->tangent;

  for (int i=0; i<NperturbPCC; i++)
  {
    pts[i] = ((dbdet_CC_curve_model_perturbed*)cm1)->pts[i];

    cv_bundles[i] = vgl_clip(((dbdet_CC_curve_model_perturbed*)cm1)->cv_bundles[i], ((dbdet_CC_curve_model_perturbed*)cm2)->cv_bundles[i], vgl_clip_type_intersect);
    if (cv_bundles[i].num_sheets()>1)
...
//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_CC_curve_model_perturbed* dbdet_CC_curve_model_perturbed::intersect(dbdet_CC_curve_model_perturbed* cm)
{
  return new dbdet_CC_curve_model_perturbed(this, cm);
}

//: is this bundle valid?
bool dbdet_CC_curve_model_perturbed::bundle_is_valid()
{
  bool valid = false;
...
void dbdet_CC_curve_model_perturbed::compute_perturbed_position_of_an_edgel(dbdet_edgel* e,
                                                                            int per_ind,
                                                                            double dpos, double /*dtheta*/,
...
//: compute the CC curve bundle for an edgel pair at the ref edgel
void dbdet_CC_curve_model_perturbed::
compute_curve_bundle(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                     double dpos, double dtheta, double token_len, double max_k)
{
  ...
  for (int i=0; i<NperturbPCC; i++)
  {
    ...
    if (ref_e == e1){
      ...
      if (test_passed)
        dbdet_CC_curve_model::compute_curve_bundle(cv_bundles[i], params, true, dpos, dtheta, token_len, max_k);
    }
    else {
      ...
      if (test_passed)
        dbdet_CC_curve_model::compute_curve_bundle(cv_bundles[i], params, false, dpos, dtheta, token_len, max_k);
    }
  }
}
...
//: copy constructor
dbdet_CC_curve_model_3d::dbdet_CC_curve_model_3d(const dbdet_CC_curve_model_3d& other) : 
  dbdet_curve_model(other) 
{ 
  type = CC3d;
...
//: constructor 4: By transporting a curve model to another extrinsic anchor point
dbdet_CC_curve_model_3d::
dbdet_CC_curve_model_3d(dbdet_CC_curve_model_3d* cm, vgl_point_2d<double> pt, double theta):
  pt(0.0,0.0), theta(0), k(0.0)
{
  type = CC3d;
...
dbdet_CC_curve_model_3d::
dbdet_CC_curve_model_3d(dbdet_edgel* ref_e, double dpos, double dtheta, double /*token_len*/, double max_k, double /*max_gamma*/, bool adaptive):
...
//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_CC_curve_model_3d* dbdet_CC_curve_model_3d::intersect(dbdet_CC_curve_model_3d* cm)
{
  assert(cm->type==CC3d);

  return new dbdet_CC_curve_model_3d(this, cm);
}

//: construct and return a curve model of the same type by transporting it to a new extrinsic anchor
dbdet_curve_model* dbdet_CC_curve_model_3d::transport(vgl_point_2d<double> pt, double theta)
{
  return new dbdet_CC_curve_model_3d(this, pt, theta);
}

//: get the size of the bundle for computing the saliency heuristic
double dbdet_CC_curve_model_3d::get_CC_3d_bundle_area()
{
  double vol = 0.0;
...
//: determine if edgel pair is legal
bool dbdet_CC_curve_model_3d::edgel_pair_legal(dbdet_int_params &params, double tan1, double tan2)
{
  //simple orientation based pruning
  bool ret = (dbdet_dot(params.ref_dir, tan1)>0 && dbdet_dot(params.ref_dir, tan2)>0);
  return ret;
...
//: Transport given curve from the given extrinsic point-tangent to the current point-tangent
//: Transport a given curve from one anchor to another
vgl_point_2d<double> dbdet_CC_curve_model_3d::transport_CC(vgl_point_2d<double>dx_dt, double k,
                                                            vgl_point_2d<double> spt, double stheta,
                                                            vgl_point_2d<double> dpt, double dtheta,
                                                            bool & sgn_change)
{
...
//: Compute the best fit curve from the curve bundle  
vgl_point_2d<double> dbdet_CC_curve_model_3d::compute_best_fit()
{
...
//: Set the best fit curve
void dbdet_CC_curve_model_3d::set_best_fit(vgl_point_2d<double> dx_dt, double kk)
{
  double dx = dx_dt.x();
  double dt = dx_dt.y();
...
bool dbdet_CC_curve_model_3d::is_C2_with(dbdet_curve_model* cm)
{
  dbdet_CC_curve_model_3d* cm2 = (dbdet_CC_curve_model_3d*)cm;
...
bool dbdet_CC_curve_model_3d::is_C1_with(dbdet_curve_model* cm)
{
  dbdet_CC_curve_model_3d* cm2 = (dbdet_CC_curve_model_3d*)cm;
...
//: Constructor 1: From a pair of edgels
dbdet_ES_curve_model::dbdet_ES_curve_model(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                                           double dpos, double dtheta, 
                                           double token_len, double max_k, double max_gamma,
                                           bool /*adaptive*/):
...
//: constructor 3: From the intersection of two curve bundles
dbdet_ES_curve_model::dbdet_ES_curve_model(dbdet_curve_model* cm1, dbdet_curve_model* cm2) :
  pt(((dbdet_ES_curve_model*)cm1)->pt), theta(((dbdet_ES_curve_model*)cm1)->theta), k(0.0), gamma(0.0)
{
  type = ES;

  //compute the new curve bundle by intersection
  cv_bundle = vgl_clip(((dbdet_ES_curve_model*)cm1)->cv_bundle, ((dbdet_ES_curve_model*)cm2)->cv_bundle, vgl_clip_type_intersect);
}

//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_ES_curve_model* dbdet_ES_curve_model::intersect(dbdet_ES_curve_model* cm)
{
  return new dbdet_ES_curve_model(this, cm);
}

//: determine if edgel pair is legal
bool dbdet_ES_curve_model::edgel_pair_legal(dbdet_int_params &params, double /*tan1*/, double /*tan2*/)
{
...
//bool dbdet_ES_curve_model::edgel_pair_legal1(dbdet_edgel* e1, dbdet_edgel* e2, 
...
//bool dbdet_ES_curve_model::edgel_pair_legal2(dbdet_edgel* e1, dbdet_edgel* e2, 
//                                   dbdet_edgel* ref_e, vgl_polygon<double> & cb)
//{
...
////  edgels. It first forms the curve bundle for the pair and checks for any overlap 
////  with Omega (the allowed curve bundle)
//bool dbdet_ES_curve_model::edgel_pair_legal2(dbdet_int_params &params, bool first_is_ref)
//{
//  //first compute the curve bundle for this pair
...
compute_curve_bundle(vgl_polygon<double>& bundle,
                     dbdet_int_params &params, bool first_is_ref, 
                     double dpos, double dtheta, 
                     double /*token_len*/, double /*max_k*/, double /*max_gamma*/)
{
                                              ...
                                              double k, double gamma)
{
  ...
  for (unsigned i=0; i<edgel_chain.size(); i++){
    ...
    double ref_dir = dbdet_vPointPoint(pt, edgel_chain[i]->pt); //reference dir
    if (dbdet_dot(ref_dir, theta)<0 || dbdet_dot(ref_dir, edgel_chain[i]->tangent)<0)
...
//: report accuracy of measurement
void dbdet_ES_curve_model::report_accuracy(double *estimates, double *min_estimates, double *max_estimates)
{
...
//: Constructor 1: From a pair of edgels
dbdet_ES_curve_model_perturbed::dbdet_ES_curve_model_perturbed(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                                                               double dpos, double dtheta, 
                                                               double token_len, double max_k, double max_gamma,
                                                               bool /*adaptive*/): 
  dbdet_ES_curve_model()
{
  //construct the curve bundle from the pair of edgels and the given uncertainty
  compute_curve_bundle(e1, e2, ref_e, dpos, dtheta, token_len, max_k, max_gamma);
...
//: constructor 3: From the intersection of two curve bundles
dbdet_ES_curve_model_perturbed::dbdet_ES_curve_model_perturbed(dbdet_curve_model* cm1, dbdet_curve_model* cm2): 
  dbdet_ES_curve_model()
{
  type = ES;
  ...
  {
    pts[i] = ((dbdet_ES_curve_model_perturbed*)cm1)->pts[i];
    tangents[i] = ((dbdet_ES_curve_model_perturbed*)cm1)->tangents[i]; 
    cv_bundles[i] = vgl_clip(((dbdet_ES_curve_model_perturbed*)cm1)->cv_bundles[i], ((dbdet_ES_curve_model_perturbed*)cm2)->cv_bundles[i], vgl_clip_type_intersect);
    if (cv_bundles[i].num_sheets()>1)
      cv_bundles[i] = vgl_polygon<double>(); //remove these erroneous curve bundles
  }
}
...
//: construct and return a curve model of the same type by intersecting with another curve bundle
dbdet_ES_curve_model_perturbed* dbdet_ES_curve_model_perturbed::intersect(dbdet_ES_curve_model_perturbed* cm)
{
  return new dbdet_ES_curve_model_perturbed(this, cm);
}

//: is this bundle valid?
bool dbdet_ES_curve_model_perturbed::bundle_is_valid()
{
  bool valid = false;
...
//: find the perturbed position and tangent given the index
void dbdet_ES_curve_model_perturbed::compute_perturbed_position_of_an_edgel(dbdet_edgel* e,
                                                                            int per_ind,
                                                                            double dpos, double dtheta,
...
//: compute the ES curve bundle for an edgel pair at the ref edgel
void dbdet_ES_curve_model_perturbed::
compute_curve_bundle(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_edgel* ref_e, 
                     double dpos, double dtheta, 
                     double token_len, double max_k, double max_gamma)
{
  ...
  for (int i=0; i<Nperturb; i++)
  {
    ...
    if (ref_e == e1){
      ...
      if (test_passed)
        dbdet_ES_curve_model::compute_curve_bundle(cv_bundles[i], params, true, dpos, dtheta, token_len, max_k, max_gamma);
    }
    else {
      ...
      if (test_passed)
        dbdet_ES_curve_model::compute_curve_bundle(cv_bundles[i], params, false, dpos, dtheta, token_len, max_k, max_gamma);
    }
  }
}
