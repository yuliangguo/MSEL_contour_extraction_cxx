// This is dbdet_edgel_link_graph.h
#ifndef dbdet_edgel_link_graph_h
#define dbdet_edgel_link_graph_h
//:
//\file
...
struct dbdet_link {
  dbdet_edgel* pe;                  ///< parent edgel
  dbdet_edgel* ce;                  ///< child edgel
  ...
  vcl_list<dbdet_curvelet*> curvelets; ///< list of curvelets containing this link
  
  //: constructor
  dbdet_link(dbdet_edgel* e1, dbdet_edgel* e2): pe(e1), ce(e2), vote(0.0), deg_overlap(0), flag(false), curvelets(0){}
  ...
  void prune_redundant_curvelets()
  {
    ...
    for (; cv_it != curvelets.end(); cv_it++)
    {
      ...
      {
        dbdet_curvelet* dup_cvlet = (*cv_it2);
...
typedef vcl_list<dbdet_link*> dbdet_link_list;
typedef vcl_list<dbdet_link*>::iterator dbdet_link_list_iter;
...
//  edgel
class dbdet_edgel_link_graph
{
public:
  vcl_vector<dbdet_link_list> cLinks; ///< child links
  vcl_vector<dbdet_link_list> pLinks; ///< parent links
  ...
  vcl_vector<dbdet_link_list> cLinks2; ///< child links
  vcl_vector<dbdet_link_list> pLinks2; ///< parent links
  
  //: constructor
  dbdet_edgel_link_graph(int size=0): cLinks(size), pLinks(size), linked(size), cLinks2(size), pLinks2(size){}

  //: destructor
  ~dbdet_edgel_link_graph()
  {
  ...
  void clear()
  {
    ...
    for (unsigned i=0; i<cLinks.size(); i++){
      dbdet_link_list_iter l_it = cLinks[i].begin();
  ...
  bool edgel_is_legal_first_edgel(dbdet_edgel* eA)
  {
  ...
  bool edgel_is_legal(dbdet_edgel* eA)
  {
    return (!linked[eA->id] &&            //not yet part of any edgel chain
  ...
  dbdet_link* are_linked(dbdet_edgel* e1, dbdet_edgel* e2)
  {
    dbdet_link_list_iter cit = cLinks[e1->id].begin();
    for(; cit!=cLinks[e1->id].end(); cit++){
      if ((*cit)->ce == e2)
        return (*cit); //return link
    }
    dbdet_link_list_iter cit1 = cLinks[e2->id].begin();
  ...
  void link(dbdet_edgel* e1, dbdet_edgel* e2, dbdet_curvelet* cvlet)
  {
    dbdet_link* link = are_linked(e1, e2);
    if (!link){
      //create a link
      dbdet_link* new_link = new dbdet_link(e1, e2);
  ...
  void remove_link(dbdet_link* link)
  {
