// This is brcv/dbdet/edge/dbdet_nms.cxx
...
dbdet_nms::dbdet_nms(): 
  thresh_(0.0), 
  parabola_fit_type_(dbdet_nms_params::PFIT_3_POINTS),
  margin_(1),
  rel_thresh_(2.5),
...
dbdet_nms::dbdet_nms(const dbdet_nms_params& nsp, const vil_image_view<double>& dir_x, 
...
void dbdet_nms::apply(bool collect_tokens,
  ...
  for (unsigned x = margin_; x < grad_mag_.ni()-margin_; x++){
    for (unsigned y = margin_; y < grad_mag_.nj()-margin_; y++)
    {
      ...
      //{
        ...
        double s_star = (parabola_fit_type_ == dbdet_nms_params::PFIT_3_POINTS) ? 
...
int dbdet_nms::intersected_face_number(const vgl_vector_2d<double>& direction)
{
...
double dbdet_nms::intersection_parameter(const vgl_vector_2d<double>& direction, int face_num)
{
...
void dbdet_nms::f_values(int x, int y, const vgl_vector_2d<double>& direction, double s, int face_num, double *f)
{
  int corners[4];
...
void dbdet_nms::get_relative_corner_coordinates(int face_num, int *corners)
{
  switch (face_num)
  {
   case 1:
...
double dbdet_nms::subpixel_s(double *s, double *f, double &max_f, double &max_d)
{
...
double dbdet_nms::subpixel_s(int x, int y, const vgl_vector_2d<double>& direction, double &max_f)
{
  double d;
  double s;
  double f;
...
void dbdet_nms::find_distance_s_and_f_for_point(int x, int y, vgl_homg_line_2d<double> line,
