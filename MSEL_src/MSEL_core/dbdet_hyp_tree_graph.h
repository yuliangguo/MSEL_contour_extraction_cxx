// This is dbdet_hyp_tree_graph.h
#ifndef dbdet_hyp_tree_graph_h
#define dbdet_hyp_tree_graph_h
//:
//\file
...
class dbdet_hyp_tree_node
{
public:
  int tree_id; //id of the hyp tree it belongs to
  bool dir;   //direction of the tree (forward or reverse)
  dbdet_curvelet* cvlet;
  dbdet_hyp_tree_node* parent; //parent node (needed for tree traversal)
  vcl_list<dbdet_hyp_tree_node*> children;

  //: constructors
  dbdet_hyp_tree_node(int id=-1, bool newdir=true): tree_id(id), dir(newdir), cvlet(0), parent(0){}
  dbdet_hyp_tree_node(dbdet_curvelet* cvlet_, int id=-1, bool newdir=true): tree_id(id), dir(newdir), cvlet(cvlet_), parent(0){}
  ...
  dbdet_hyp_tree_node* add_child(dbdet_curvelet* cvlet)
  {
    dbdet_hyp_tree_node* new_node = new dbdet_hyp_tree_node(cvlet, tree_id, dir);
...
class dbdet_hyp_tree
{
public:
  int tree_id;
  dbdet_hyp_tree_node* root;
  bool resolved;
  dbdet_hyp_tree_node* best_path; 
  double least_cost;

  //: default constructor
  dbdet_hyp_tree(int id=-1): tree_id(id), root(0), resolved(false), best_path(0), least_cost(1000)  {}

  //: constructor given a root node
  dbdet_hyp_tree(dbdet_hyp_tree_node* new_root, int id=-1) : 
    tree_id(id), root(new_root), resolved(false), best_path(0), least_cost(1000) {}

  //: destructor
  ~dbdet_hyp_tree(){ delete_tree(); }
  ...
  class iterator
  {
    public:
      ...
      iterator(dbdet_hyp_tree_node* ptr) : ptr_(ptr) { if (ptr) cur_path_.push_back(ptr->cvlet); }
      //: copy constructor
      iterator(const iterator& other) : ptr_(other.ptr_), cur_path_(other.cur_path_) {}
      
      //operators
      dbdet_hyp_tree_node* operator*(){ return ptr_; }
      ...
      void operator++(int) //post increment operator
      {
        ...
        if (ptr_->children.size()==0){
          dbdet_hyp_tree_node* cur = ptr_;
          dbdet_hyp_tree_node* parent = cur->parent;
          ...
          vcl_list<dbdet_hyp_tree_node*>::iterator nit = parent->children.begin();
      ...
      vcl_vector<dbdet_curvelet*>& get_cur_path() { return cur_path_; }

    protected:
      dbdet_hyp_tree_node* ptr_;             //this is the node that the iterator is currently pointing to
      vcl_vector<dbdet_curvelet*> cur_path_; // this is the path from point where the iterator was initialized to the current node
  };
  ...
  {
    dbdet_hyp_tree_node* node_to_del = *it;
  ...
  void print_all_paths()
  {
    ...
    for ( ; pit != end(); pit++){
      if ((*pit)->is_leaf()){
        ...
        vcl_vector<dbdet_curvelet*> CF_list = pit.get_cur_path();
  ...
  void next_node(dbdet_hyp_tree_node* cur_node, vcl_list<dbdet_curvelet*> &CF)
  {
    ...
    if (cur_node->children.size()==0){
      //print out the current CF list
      vcl_cout << ":: ";
      vcl_list<dbdet_curvelet*>::iterator cvit = CF.begin();
    ...
    //go to its children
    vcl_list<dbdet_hyp_tree_node*>::iterator nit = cur_node->children.begin();
...
class dbdet_HT_graph
{
public:
  vcl_vector<dbdet_hyp_tree*> nodes;
  ...
  dbdet_HT_graph():nodes(0), CPL_links(0), CPT_links(0){}
  ~dbdet_HT_graph(){ clear(); }
  ...
  void insert_node(dbdet_hyp_tree* new_node){nodes.push_back(new_node);  }
...
class dbdet_edgel_labels
{
public:
  vcl_vector<vcl_set<dbdet_hyp_tree_node*> > labels; //contains the hyp tree nodes
  vcl_vector<bool> claimed;                          //flags to mark edgels that ahve been claimed already

  dbdet_edgel_labels(): labels(0), claimed(0){}
  ~dbdet_edgel_labels(){ labels.clear(); claimed.clear(); }
...
class dbdet_HTG_link_path
{
public:
  dbdet_hyp_tree* src;
  dbdet_hyp_tree* tgt;
  double cost;
  vcl_vector<dbdet_curvelet*> cvlets;

  dbdet_HTG_link_path(dbdet_hyp_tree* sHT=0, dbdet_hyp_tree* tHT=0): src(sHT), tgt(tHT), cost(0.0), cvlets(0) {}

};

inline bool path_metric_less_than(dbdet_HTG_link_path* p1, dbdet_HTG_link_path* p2)
{
  return p1->cost < p2->cost;
}

#endif // dbdet_hyp_tree_graph_h
