...
for f = 1: length(src_files)
    f
    % load contour maps
    fprintf('load file\n');
    src = [src_path, src_files(f).name];
    d2=load_contours_gt_smooth(src);
...
% write new contour map files
    ...
    for j=1:size(edge_map,1)
      ...
      s = ['(',num2str(edge_map(j,1)),', ', num2str(edge_map(j,2)),')  ',num2str(edge_map(j,3)),'   2   0'];
      fprintf(fout, '%s\n', s);
    end
