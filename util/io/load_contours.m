function [cem, edg, cf_idx] = load_contours(filename)
...
while 1,
    ...
    if (strncmp(lineBuffer, '[Contours]', length('[Contours]')))
        ...
        contours = cell(num_contours,0);
        %contours containing edge index
        cf_idx = cell(num_contours,0);    
        ...
        for i=1:num_contours,
            ...
            cf_idx{i} = e_ids+1; % this is the matlab index
...
%             end
    ...
    if (strncmp(lineBuffer, '[Contour Properties]', length('[Contour Properties]')))
        ...
        con_props = zeros(num_contours, 8);
        for i=1:num_contours,
